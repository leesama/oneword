<template>
  <div class="third-step">
    <the-header @close="handleClose" :transparent="false" @rightClick="finish">
      <template #left>
        <i class="iconfont">&#xe612;</i>
      </template>
...
export default {
  ...
  methods: {
    ...
    async finish() {
      // 拼接type字段
      const type = `one_${this.isRadius ? 0 : 1}_${this.direction ? 0 : 1}_${
        this.fontCode
      }_${this.isCenter ? 0 : 1}`
      let formData = new FormData()
      for (const i in this.cardInfo) {
        formData.append(i, this.cardInfo[i])
      }
      const imgData = formData.get('picpath')
      if (imgData) {
        formData.set('picpath', base64ToFile(imgData, 'pic.jpg'))
      } else {
        formData.delete('picpath')
      }
      formData.append('type', type)

      const { code, msg } = await newCard(formData)
      if (code === 0) {
        this.$toastMessage('创建成功')
        this.$router.back(-1)
      } else {
